...
import { whenToolsIdle } from "./results";
...
try {
	...
	const close = (drain: boolean) => {
		...
		// A host that closes stdin may still be waiting for answers to what it already sent.
		const drained = drain ? pause().then(whenToolsIdle).then(pause) : Promise.resolve();
