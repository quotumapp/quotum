...
localDescribe("platform project identity persistence", () => {
	...
	it("installs the complete constrained and indexed platform ownership schema", async () => {
		...
		expect(tables.map((row) => row.table_name)).toEqual([
			...
			"platform_auth_jwks",
			"platform_auth_links",
			"platform_auth_oauth_access_tokens",
			"platform_auth_oauth_client_assertions",
			"platform_auth_oauth_client_resources",
			"platform_auth_oauth_clients",
			"platform_auth_oauth_consents",
			"platform_auth_oauth_refresh_tokens",
			"platform_auth_oauth_resources",
